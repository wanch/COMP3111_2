...
import hkust.cse.calendar.system.NotificationCentre;
...
public class CalGrid extends JFrame implements ActionListener {
	...
	public LocationStorageController locationController;
	...
	private JMenu dateStatus = new JMenu("Time Machine");
	
	private JMenuItem intivationMenuItem;
	...
	public CalGrid(ApptStorageControllerImpl con) {
		...
		notificationCentre.setApptController(controller);
				...
				if (tem.equals("") == false) {
					try {
						...
						if (today.get(Calendar.YEAR) == currentY &&
							...
							today.get(today.DAY_OF_MONTH) == Integer.parseInt(tem)) {
							return new CalCellRenderer(today, hasAppt);
						}
						else {
							return new CalCellRenderer(null, hasAppt);
						}
					} catch (Throwable e) {
						System.exit(1);
					}
	...
	JMenuBar createMenuBar() {
		...
		ActionListener listener = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (e.getActionCommand().equals("New Appointment")) {
					...
					locationManager = new LocationManager(controller);
				}
				else if(e.getActionCommand().equals("Manage Users")) {
					userManager = new UserManager();
				}
				else if(e.getActionCommand().equals("Change Time")) {
					TimeMachineDialog tm = new TimeMachineDialog(CalGrid.this);
					tm.show();
				}
				else if(e.getActionCommand().equals("Manage Appointments")) {
					UserApptManager userApptManager = new UserApptManager(CalGrid.this, CalGrid.this.controller);
				}
			}
		};
		...
		if(mCurrUser.getRole().equals("Admin")) {
			...
			mi.addActionListener(listener);
			adminMenu.add(mi);
			mi = new JMenuItem("Manage Appointments");
			mi.addActionListener(listener);
			adminMenu.add(mi);
		}
		else
	...
	public void UpdateCal() {
		if (mCurrUser != null && tableView != null) {
	...
	public void initalTodayReminder() {
		Integer temp;
		
		Timestamp start = new Timestamp(0);
		start.setYear(today.get(Calendar.YEAR));
		start.setMonth(today.get(Calendar.MONTH)+1);
		start.setDate(today.get(Calendar.DATE));
		start.setHours(0);
		start.setMinutes(0);
		start.setSeconds(0);
		
		Timestamp end = new Timestamp(0);
		end.setYear(today.get(Calendar.YEAR));
		end.setMonth(today.get(Calendar.MONTH)+1);
		end.setDate(today.get(Calendar.DATE));
		end.setHours(23);
		end.setMinutes(59);
		end.setSeconds(59);
	...
	public void updateDisplay() {
		...
		if(today.get(today.SECOND) < 10) {
			second = "0" + today.get(today.SECOND);
		}
		else {
			second = new Integer(today.get(today.SECOND)).toString();
		}
		
		if(today.get(today.MINUTE) < 10) {
			minute = "0" + today.get(today.MINUTE);
		}
		else {
			minute = new Integer(today.get(today.MINUTE)).toString();
		}
			
		
		if(today.get(today.HOUR_OF_DAY) == 0 && today.get(today.MINUTE) == 0 && today.get(today.SECOND) == 0) {
			UpdateCal();
		}
	...
	private void invitationOfJointAppt(){
		Appt JointApptsInWaitList[] = controller.RetrieveJointApptsInWaitlist();
		for(Appt appt : JointApptsInWaitList){
			AppScheduler waitlistwindows = new AppScheduler("Invitation", this,appt.getID());
			waitlistwindows.updateSetApp(appt);
			waitlistwindows.show();
			waitlistwindows.setResizable(false);
